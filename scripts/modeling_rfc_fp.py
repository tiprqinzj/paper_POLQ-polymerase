...
from utils_public import cal_time, load_json
...
def extract_XY(df, trtecv_title, valid_title, des_title, target):
...
def grid_search(tr_X, tr_Y, va_X, va_Y):
    ...
    params = {'n_estimators': [50, 100, 200, 400],
              ...
              'max_features': ['log2', 'sqrt', 0.25, 0.5, 1.0],
...
def build_rfc_fp_model(modeldf_file, json_file, trte_title, trtecv_title, target, des, prefix, validnum=5):
    ...
    for valid in ['train' + str(i) for i in range(1, validnum+1)]:
        ...
        tr_X, tr_Y, va_X, va_Y = extract_XY(tr_df, trtecv_title, valid, model_des, target)
    ...
    tr_X = model_df.loc[model_df.loc[:, trte_title] == 'train', model_des].values
    tr_Y = model_df.loc[model_df.loc[:, trte_title] == 'train', target].values
    te_X = model_df.loc[model_df.loc[:, trte_title] == 'test', model_des].values
    ...
    pd.concat([
        ...
        pd.DataFrame(np.around(tr_pred, 3), columns=['valid1', 'valid2', 'valid3', 'valid4', 'valid5']),
    ...
    pd.concat([
        ...
        pd.DataFrame(np.around(te_pred, 3), columns=['valid1', 'valid2', 'valid3', 'valid4', 'valid5']),
